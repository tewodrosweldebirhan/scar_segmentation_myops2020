...
class RMILoss(nn.Module):
	...
	def forward_sigmoid(self, logits_4D, labels_4D):
		...
		Args:
			...
			labels_4D 	:	[N, H, W], dtype=long
		"""
        # softmax_loss = F.cross_entropy(torch.log(logits_4D), labels_4D) #
		...
		# add together
		final_loss = (self.weight_lambda * softmax_loss + rmi_loss * (1 - self.weight_lambda) if self.lambda_way
				else softmax_loss + rmi_loss * self.weight_lambda)
